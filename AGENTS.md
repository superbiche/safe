...
- Refusals: single final stderr line; exit 100 (policy) / 102 (operator TTY needed) / 104 (audit BLOCK); 0/10/20 are `safe audit package-audit` verdict codes; 13 is the gate-mode fresh-release Socket consent ask and 14 the pending-socket-score not-all-green code (the gate resolves both to 0/100/102); 127 = genuinely missing command.
