...
PATH="$mockbin:$PATH" \
  bash -c '
    ...
    printf %s "{\"spec\":\"ackpkg@3.1.4\",\"ecosystem\":\"npm\",\"resolved_versions\":[\"3.1.4\"],\"verdict\":\"BLOCK\",\"guarddog\":{\"contribution\":\"BLOCK\",\"rules\":[\"threat-network-exfiltration\",\"threat-runtime-obfuscation-general\"],\"versions\":[{\"version\":\"3.1.4\",\"rules\":[\"threat-network-exfiltration\",\"threat-runtime-obfuscation-general\"]}]}}" \
...
SAFE_RUN_CONFIG_DIR="$tmp/config-add-ack-perversion" \
SAFE_RUN_DATA_DIR="$tmp/data-add-ack-perversion" \
SAFE_AUDIT_DATA_DIR="$tmp/audit data ws" \
SAFE_AUDIT_VERDICT=BLOCK \
SAFE_AUDIT_CALL_LOG="$tmp/audit-calls-add-ack-perversion.log" \
SAFE_RUN_PATH="$SAFE_RUN" \
PATH="$mockbin:$PATH" \
  bash -c '
    set -- version
    ...
    ensure_dirs
    mkdir -p "$SAFE_AUDIT_DATA_DIR/checks"
    # Multi-version receipt: rule r2 was emitted only for 2.5.0. Recording
    # an ack for 1.5.0 must take the PER-VERSION set, never the aggregate
    # (PR#67 F3) — and the whitespace data dir exercises PR#67 F6.
    printf %s "{\"spec\":\"ackpkg\",\"ecosystem\":\"npm\",\"resolved_versions\":[\"1.5.0\",\"2.5.0\"],\"verdict\":\"BLOCK\",\"guarddog\":{\"contribution\":\"BLOCK\",\"rules\":[\"r1\",\"r2\"],\"versions\":[{\"version\":\"1.5.0\",\"rules\":[\"r1\"]},{\"version\":\"2.5.0\",\"rules\":[\"r1\",\"r2\"]}]}}" \
      > "$SAFE_AUDIT_DATA_DIR/checks/2026-08-04-ackpkg-1.5.0.json"
    registry_integrity_npm() { printf "sha512-fixture"; }
    require_operator_tty() { :; }
    host_allow_preflight_audit() { :; }
    cmd_host_allow_add ackpkg@1.5.0 --acknowledge-behavioral --reason "fp" >/dev/null 2>&1
    [[ "$(jq -r ".packages.ackpkg.behavioral_ack.rules | join(\",\")" "$HOST_ALLOW_FILE")" == "r1" ]]
  ' safe-run || fail "per-version ack recorded sibling rules or broke on a whitespace data dir"
pass "host-allow add --acknowledge-behavioral records only the pinned version's rules (whitespace data dir)"

SAFE_RUN_CONFIG_DIR="$tmp/config-add-ack-aggregate" \
SAFE_RUN_DATA_DIR="$tmp/data-add-ack-aggregate" \
SAFE_AUDIT_DATA_DIR="$tmp/audit-data-ack-aggregate" \
SAFE_AUDIT_VERDICT=BLOCK \
SAFE_AUDIT_CALL_LOG="$tmp/audit-calls-add-ack-aggregate.log" \
SAFE_RUN_PATH="$SAFE_RUN" \
ERR_FILE="$tmp/add-ack-aggregate.err" \
PATH="$mockbin:$PATH" \
  bash -c '
    set -- version
    source "$SAFE_RUN_PATH" >/dev/null
    ensure_dirs
    mkdir -p "$SAFE_AUDIT_DATA_DIR/checks"
    printf %s "{\"spec\":\"ackpkg@3.1.4\",\"ecosystem\":\"npm\",\"resolved_versions\":[\"3.1.4\"],\"verdict\":\"BLOCK\",\"guarddog\":{\"contribution\":\"BLOCK\",\"rules\":[\"r1\"]}}" \
      > "$SAFE_AUDIT_DATA_DIR/checks/2026-08-04-ackpkg-3.1.4.json"
    registry_integrity_npm() { printf "sha512-fixture"; }
    require_operator_tty() { :; }
    host_allow_preflight_audit() { :; }
    set +e
    ( cmd_host_allow_add ackpkg@3.1.4 --acknowledge-behavioral --reason "fp" ) >/dev/null 2>"$ERR_FILE"
    rc=$?
    set -e
    [[ "$rc" -ne 0 ]]
    [[ "$(jq -r ".packages | length" "$HOST_ALLOW_FILE")" == "0" ]]
  ' safe-run || fail "aggregate-only receipt was accepted for an exact-version ack"
pass "host-allow add --acknowledge-behavioral rejects receipts without per-version rules"
