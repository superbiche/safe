...
#
# Detection mirrors gate-lib.sh `safe_gate_is_wrapper`: a generated wrapper
# carries `# safe-gate-wrapper` within its first bytes (gate-lib probes 4096).
# Kept as a small self-contained copy so the belt does not source — and so
# cannot be broken by — the gate library it exists to test.

# real_tool_is_wrapper <path> — true when <path> is a safe gate wrapper.
real_tool_is_wrapper() {
  LC_ALL=C head -c 4096 -- "$1" 2>/dev/null | tr -d '\0' \
    | grep -q '# safe-gate-wrapper'
}
