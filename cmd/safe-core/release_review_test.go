...
func reviewSpec(t *testing.T, artifact, checksums string) string {
	t.Helper()
	spec := fmt.Sprintf(`{"spec_version": 3,"subject":{"repo":"o/r","version":"v1.2.3"},
...
func TestReleaseReviewSignedSpecStillWarns(t *testing.T) {
	...
	artifact := writeReviewFile(t, dir, "tool.tar.gz", "payload")
	checksums := writeReviewFile(t, dir, "checksums.txt", digestOf("payload")+"  tool.tar.gz\n")
	spec := fmt.Sprintf(`{"spec_version": 3,"subject":{"repo":"o/r","version":"v1"},
	  "artifacts":[{"path":%q,"asset_name":"tool.tar.gz","evidence":{"checksum_file":%q,
...
func TestReleaseReviewUnusableSpec(t *testing.T) {
	cases := []struct {
		...
		{"unknown field", `{"spec_version": 3,"subject":{"repo":"o/r","version":"v1"},"artifacts":[{"path":"a"}],"x":1}`, `unknown field "x"`},
		{"release enabled without an asset", `{"spec_version": 3,"subject":{"repo":"o/r","version":"v1"},"artifacts":[{"path":"a"}],"checks":{"release":{"enabled":true}}}`, "checks.release.asset is required"},
		{"no checks enabled", `{"spec_version": 3,"subject":{"repo":"o/r","version":"v1"},"artifacts":[{"path":"a"}],"checks":{}}`, "no checks are enabled"},
		{
			name:    "object appended after the spec",
			spec:    `{"spec_version": 3,"subject":{"repo":"o/r","version":"v1"},"artifacts":[{"path":"a","evidence":{"checksum_file":"c"}}]}{"unknown":true}`,
			wantSub: "exactly one JSON document",
		},
		{
			name:    "spec_version given twice",
			spec:    `{"spec_version":3,"subject":{"repo":"o/r","version":"v1"},"artifacts":[{"path":"a","evidence":{"checksum_file":"c"}}],"spec_version": 3}`,
