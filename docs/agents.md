...
| 11 | Gate-mode only (`safe audit package-audit --gate install`): verdict WARN whose causes are ENTIRELY audit-infrastructure outages (Socket/OSV unreachable) with no adverse package evidence. Not a package finding — a missing signal. Plain `safe audit package-audit` still exits 10 for the same state; only the gate distinguishes it. | Via the gate this is operator-overridable only by a deliberate confirmation at an interactive terminal — the gate prompts and, if accepted, proceeds; non-interactively it refuses with exit 102. Exit 11 itself has no silent acceptance path: an agent cannot turn it into an install with --yes or a host-allow entry. (A standing install.auto_allow_tolerate that already lists an infra cause can still intercept the WARN BEFORE it reaches exit 11 — a pre-existing behavior the terminus ruling disfavors, pending a separate operator decision.) Prefer restoring the tier (socket login / wait / safe doctor); escalate to the operator to decide. |
