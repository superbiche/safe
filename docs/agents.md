...
**Audited subcommands.** Installs (install/add/ci/require and global variants), updates (update/up/upgrade, npm it), and exec-style fetch-and-run (npm exec|x, pnpm dlx, pnpx, yarn dlx, bun x, uv run --with|-w, uv tool run, go run <module>@<version>). npm and Composer aliases and command abbreviations that resolve to these gated routes are audited too, including Composer's abbreviated global proxy; a gated global route scans its existing global-home project and any effective first --working-dir project. Safe preserves the original command token on every path that reaches delegation; for a gated-looking ambiguous token, it may refuse before delegation rather than letting the package manager emit its ambiguity error. An unrecognized space-form value flag before the command fails closed (escapable with --flag=value).
